...
//___________________________________________________________________________//
  ...
  TransiEnt.Basics.Tables.Ambient.Temperature_Hamburg_900s_2012 tamb(datasource=TransiEnt.Basics.Tables.DataPrivacy.isPublic, relativepath="ambient/Temperature_Hamburg-Fuhlsbuettel_3600s_01012012_31122012.txt") annotation (Placement(transformation(extent={{-107,-10},{-87,10}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput m_flow_set "Setpoint for total mass flow"
                                   annotation (Placement(transformation(
          rotation=0, extent={{90,50},{110,70}})));
  Modelica.Blocks.Interfaces.RealOutput Q_flow_set "Setpoint for total heat production"
                                         annotation (Placement(transformation(
          rotation=0, extent={{90,-70},{110,-50}})));
  Modelica.Blocks.Interfaces.RealOutput T_feed_set "Setpoint for feed flow temperature"
                                         annotation (Placement(transformation(
          rotation=0, extent={{90,10},{110,30}})));
  Modelica.Blocks.Interfaces.RealOutput T_return_set "Setpoint for return flow temperature"
                                           annotation (Placement(transformation(
...
equation
  ...
  connect(sign_changer.y, Q_flow_set) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(T_supply_return_target.T_amb, m_flow_target.u1) annotation (Line(points={{-10,0},{-40,0},{-40,80},{40,80},{40,63},{55,63}}, color={0,0,127}));
  ...
  connect(T_supply_return_target.T_amb, heatingLoadCharline.Q_flow) annotation (Line(points={{-10,0},{-45,0}}, color={0,0,127}));
  connect(sign_changer.u, heatingLoadCharline.Q_flow) annotation (Line(points={{44,-60},{-40,-60},{-40,0},{-45,0}}, color={0,0,127}));
  connect(tamb.y1, heatingLoadCharline.T_amb) annotation (Line(points={{-86,0},{-78,0},{-78,1},{-67,1}}, color={0,0,127}));
  annotation (Diagram(coordinateSystem(extent={{-110,-100},{90,100}},
