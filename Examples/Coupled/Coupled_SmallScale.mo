...
//___________________________________________________________________________//
  ...
  TransiEnt.Components.Gas.VolumesValvesFittings.PipeFlow_L4_Simple_varXi pipe1(
    ...
    length=1000,
    N_cv=4) annotation (Placement(transformation(extent={{60,94},{32,104}})));
  TransiEnt.Components.Gas.VolumesValvesFittings.PipeFlow_L4_Simple_varXi pipe2(
    frictionAtInlet=true,
    frictionAtOutlet=true,
    length=1000,
    N_cv=4,
    redeclare model PressureLoss = ClaRa.Basics.ControlVolumes.Fundamentals.PressureLoss.VLE_PL.QuadraticNominalPoint_L4) annotation (Placement(transformation(extent={{32,12},{60,22}})));
  TransiEnt.Components.Gas.VolumesValvesFittings.PipeFlow_L4_Simple_varXi pipe3(
    ...
    N_cv=4) annotation (Placement(transformation(
        ...
        rotation=90,
        origin={86,59})));
  TransiEnt.Components.Gas.VolumesValvesFittings.RealGasJunction_L2 junction annotation (Placement(transformation(extent={{-4,28},{16,8}})));
  TransiEnt.Components.Gas.VolumesValvesFittings.RealGasJunction_L2 junction1 annotation (Placement(transformation(extent={{16,88},{-4,108}})));
  TransiEnt.Components.Gas.VolumesValvesFittings.RealGasJunction_L2 junction2 annotation (Placement(transformation(extent={{96,88},{76,108}})));
  TransiEnt.Components.Gas.VolumesValvesFittings.RealGasJunction_L2 junction3 annotation (Placement(transformation(extent={{76,26},{96,6}})));
  ...
  TransiEnt.Components.Gas.VolumesValvesFittings.RealGasJunction_L2 junction4 annotation (Placement(transformation(extent={{-68,28},{-48,8}})));
  ...
  TransiEnt.Producer.Combined.SmallScaleCHP.CHP_ice CHP(
    ...
    T_init=293.15,
    p_init=80000000000) annotation (Placement(transformation(extent={{38,-120},{-28,-58}})));
  ...
  ClaRa.Components.BoundaryConditions.BoundaryVLE_Txim_flow boundaryVLE_Txim_flow(
    ...
    variable_T=true,
    m_flow_const=16) annotation (Placement(transformation(extent={{-82,-92},{-62,-72}})));
  ClaRa.Components.BoundaryConditions.BoundaryVLE_phxi boundaryVLE_phxi(p_const=6e5) annotation (Placement(transformation(extent={{-130,-70},{-110,-50}})));
  TransiEnt.Components.Boundaries.Heat.Heatflow_L2 heatFlow(
    m_flow_nom=16,
    Q_flow_n=2.1e6,
    T_start=333.15) annotation (Placement(transformation(extent={{-64,-60},{-84,-40}})));
  TransiEnt.Components.Sensors.TemperatureSensor temperatureSensor annotation (Placement(transformation(extent={{-112,-72},{-92,-92}})));
  Modelica.Blocks.Sources.Sine Q_flow_consumer(
    freqHz=2/86400,
    offset=1500e3,
    amplitude=800e3) annotation (Placement(transformation(extent={{-46,-44},{-60,-30}})));
...
equation
  connect(junction1.gasPort1, pipe1.gasPortOut) annotation (Line(
      points={{16,98},{32,98},{32,99}},
      color={255,255,0},
      thickness=1.5));
  connect(junction1.gasPort2, pipe.gasPortOut) annotation (Line(
      points={{6,88},{6,71}},
      color={255,255,0},
      thickness=1.5));
  connect(pipe.gasPortIn, junction.gasPort2) annotation (Line(
      points={{6,43},{6,28}},
      color={255,255,0},
      thickness=1.5));
  connect(junction.gasPort3, pipe2.gasPortIn) annotation (Line(
      points={{16,18},{32,18},{32,17}},
      color={255,255,0},
      thickness=1.5));
  connect(pipe2.gasPortOut, junction3.gasPort1) annotation (Line(
  ...
  connect(junction2.gasPort2, pipe3.gasPortIn) annotation (Line(
      ...
      color={255,255,0},
      thickness=1.5));
  connect(pipe1.gasPortIn, junction2.gasPort3) annotation (Line(
  ...
  connect(boundaryVLE_Txim_flow.steam_a, CHP.waterPortIn) annotation (Line(
      points={{-62,-82},{-62,-82},{-46,-82},{-46,-67.61},{-28,-67.61}},
      color={0,131,169},
      thickness=0.5));
  ...
  connect(temperatureSensor.T, boundaryVLE_Txim_flow.T) annotation (Line(points={{-91,-82},{-84,-82}},    color={0,0,127}));
