...
//___________________________________________________________________________//
  ...
  Modelica.Blocks.Sources.Constant P_nom(k=1e6) annotation (Placement(transformation(extent={{-80,40},{-60,60}})));
  Modelica.Blocks.Sources.Constant P_load(k=0.5e6) annotation (Placement(transformation(extent={{40,40},{60,60}})));
  Modelica.Blocks.Sources.Constant w_nom(k=2*3.14*1/5)
                                              "rad/sec" annotation (Placement(transformation(extent={{-40,40},{-20,60}})));
  ...
  Modelica.Blocks.Continuous.LimPID T_set(
    yMax=2*P_nom.k/w_nom.k,
    wp=1,
    k=P_nom.k/w_nom.k/w_nom.k*10,
    controllerType=Modelica.Blocks.Types.SimpleController.PI)
                                  "Time constant"                           annotation (Placement(transformation(extent={{-28,-4},{-8,16}})));
  TransiEnt.Components.Boundaries.Mechanical.Power Turbine annotation (Placement(transformation(extent={{-78,-63},{-58,-41}})));
  Modelica.Mechanics.Rotational.Components.Inertia Inertia(J=H_gen.k*w_nom.k^2/P_nom.k, w(fixed=true, start=w_nom.k))
                                                                  annotation(Placement(transformation(extent={{-48,-63},{-28,-43}})));
  Modelica.Blocks.Sources.Constant f_set(k=w_nom.k/2/Modelica.Constants.pi) "Hz" annotation (Placement(transformation(extent={{-72,-4},{-52,16}})));
...
equation
  connect(Power.epp, Generator.epp) annotation (Line(
      ...
      color={0,135,135},
      thickness=0.5));
  connect(T_set.y, Generator.T_set) annotation (Line(points={{-7,6},{-2,6},{11.7,6},{11.7,-42.1}},color={0,0,127}));
  connect(Inertia.flange_b, Generator.mpp) annotation (Line(points={{-28,-53},{-14,-53},{-14,-52.5},{1.5,-52.5}}, color={0,0,0}));
