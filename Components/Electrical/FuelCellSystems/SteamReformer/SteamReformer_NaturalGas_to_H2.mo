...
//___________________________________________________________________________//
  ...
  parameter Modelica.SIunits.Pressure p_reformer = 1.013e5 "Druck im Reformer";
  parameter Modelica.SIunits.Density d_kat = 1900 "Katalysatordichte";
  parameter Real scale_kat = 1 "Einflussfaktor auf die Reaktionswrme mit der Annahme, dass mehr Reaktionen ablaufen, als durch Konzentrationsnderung wahrgenommen";
  parameter Real eps = 1e-6 "Genauigkeit der Berechnung";
  parameter Modelica.SIunits.Volume V_reac = 0.001 "Volumen des Reaktors";
  parameter Real eps_kat = 0.4 "Porositt der Katalysatorschttung";

  // === Order of components in vectors: {CH4 , CO , H2 , H2O , CH4 , O2} ===

  parameter Real K_oi[4] = {6.65e-4, 8.23e-5, 6.12e-9, 1.77e5} "Adsoprtionskoeffizientskonstante der Komponente i {CH4, CO, H2, H2O}";
  parameter Real dH_i[4] = {-38280, -70650, -82900, 88680} "Vektor mit den Enthalpien der Komponente i {CH4, CO, H2, H2O}";

  // === Order of components: {CH4 , O2}

  parameter Real K_oiC[2] = {1.26e-1, 7.78e-7} "Adsoprtionskoeffizientskonstante der combusteten Komponente i {CH4-c, O2-c}";
  parameter Real dH_iC[2] = {-27300, -92800} "Vektor mit den Enthalpien der Verbrennung Komponente i {CH4-c, O2-c}";

 // === Order of components: Reaction {1  ,  2  ,  3  ,  4a  ,  4b}

  parameter Real k_oi[5] = {1.17e15, 2.83e14, 5.43e5, 8.11e5, 6.82e5} "k_oi";
  parameter Real E_j[5] = {240100, 243900, 67130, 86000, 86000} "Aktivierungsnergien";
  parameter Real Delta_H_R[4] = {206.2*1000, 164.9*1000, -41.1*1000, -802.7*1000} "Reaktionswrmen der Reaktionen 1 bis 4 in J/mol";
  ...
  parameter Real x_i_start[7] = {0.35,0.1,0,0.45,0.00001,0,0.1} "Start values for mass fractions";
  // _____________________________________________
  //
  //                 Outer Models
  // _____________________________________________
  ...
  Real k_i[5] "Reaktionskoeffizient der Reaktion i";
  Real K_I = exp(-26830/T_reformer + 30.114) "Gleichgewichtskonstante der ersten Gleichung in [bar]";
  Real K_III = exp(4400/T_reformer - 4.036) "Gleichgewichtskonstante der dritten Gleichung in [-]";
  Real K_II = K_I * K_III "Gleichgewichtskonstante der zweiten Gleichung in [bar]";
  Real r_i[6] "Reaktionsrate der Komponenten {CH4, O2, CO2, H2O, H2, CO}";
  Real A[6,4] = [-1, -1, 0, -1; 0, 0, 0, -2; 0, 1, 1, 1; -1, -2, -1, 2; 3, 4, 1, 0; 1, 0, -1, 0] "Matrix zur Berechnung der Reaktionsraten";
  Real b_running[4] = {0.07*R1, 0.06*R2, 0.7*R3, 0.05*R4};
  Real b_preheat[4] = {0, 0, 0, 0};
  Real b[4];
  Real K_iC[2] "Adsoprtionskoeffizient der combusteten Komponente i {CH4-c, O2-c}";
  Real OMEGA "Omega halt";
  Real R1 "Reaktionsrate 1";
  Real R2 "Reaktionsrate 2";
  Real R3 "Reaktionsrate 3";
  Real R4 "Reaktionsrate 4";
  Real K_i[4] "Adsoprtionskoeffizient der Komponente i {CH4, CO, H2, H2O}";
  Real x_i[7]( start = x_i_start) "Mole der Gasmischung in dem Reformer whrend der Reaktion";
  Modelica.SIunits.MassFraction xi_ch4 "Massenanteil CH4";
  Modelica.SIunits.MassFraction xi_o2 "Massenanteil O2";
  Modelica.SIunits.MassFraction xi_co2 "Massenanteil CO2";
  Modelica.SIunits.MassFraction xi_h2o "Massenanteil H2O";
  Modelica.SIunits.MassFraction xi_h2 "Massenanteil H2";
  Modelica.SIunits.MassFraction xi_co "Massenanteil CO";
  Modelica.SIunits.MoleFraction x_n2 "Molanteil N2";
  Modelica.SIunits.Pressure p_ch4 = sg.p_i[1]/1e5 "Massenanteil CH4";
  Modelica.SIunits.Pressure p_o2 = sg.p_i[2]/1e5 "Massenanteil 02";
  Modelica.SIunits.Pressure p_co2 = sg.p_i[3]/1e5 "Massenanteil CO2";
  Modelica.SIunits.Pressure p_h2o = sg.p_i[4]/1e5 "Massenanteil H2O";
  Modelica.SIunits.Pressure p_h2 = sg.p_i[5]/1e5 "Massenanteil H2";
  Modelica.SIunits.Pressure p_co = sg.p_i[6]/1e5 "Massenanteil CO";
  Real m_ges;
  Modelica.SIunits.Temperature T_reformer(start = simCenter.T_amb_const) "Temperatur des Reformerabschnittes";
  Modelica.SIunits.Temperature T_gas = (T_reformer + START.T)/2 "Annahme mittlere Temperatur zischen Reformer und Eintrittstemperatur";
  Modelica.SIunits.MassFlowRate m_flow = feed.m_flow "Massenstrom der Gasmischung durch den Reformer";
  Modelica.SIunits.HeatFlowRate Q_reac;
...
equation
  ...
  // _____________________________________________
      ...
      //Term um die neuen Konzentrationen in eine Massenzusammensetzung umzurechnen, die dann wiederum TILMedia beeinflussen
      ...
      der(T_reformer) = 1/(m * cp) * ( heatport.Q_flow - m_flow*(sg.h - START.h)   + Q_reac); //

      // Reaktionswrme
      Q_reac = -(b[1]*Delta_H_R[1] + b[2]*Delta_H_R[2] + b[3]*Delta_H_R[3] + b[4]*Delta_H_R[4])*d_kat*V_reac*scale_kat;
      ...
      // enthalpy (keine Wrmebergange (dynamisch) zwischen Reformer und Gas betrachtet"
