...
model TestStratifiedHotWaterStorage_L1 "Minimal test model for stratified hot water storage L1 model"
  import TransiEnt;
...
//___________________________________________________________________________//
  ...
  Modelica.Blocks.Sources.Step T_gen(
    height=10,
    offset=273.15 + 80,
    startTime=4*3600) annotation (Placement(transformation(extent={{-100,-20},{-80,0}})));
  Modelica.Blocks.Sources.Step MassFlowStep(
    height=-0.02,
    offset=0.2,
    startTime=15*3600) annotation (Placement(transformation(extent={{96,-8},{76,12}})));
  TransiEnt.Storage.Heat.HotWaterStorage_L2 Storage annotation (Placement(transformation(extent={{-24,-64},{28,-20}})));
equation
  ...
  connect(Storage.ConOut, SinkConsumerSide.steam_a) annotation (Line(
      points={{20.2,-23.96},{32,-23.96},{32,-60},{50,-60}},
      color={175,0,0},
      thickness=0.5,
      smooth=Smooth.None));
  ...
  connect(Storage.ConIn, SourceConsumerSide.steam_a) annotation (Line(
      points={{16.56,-59.6},{36,-59.6},{36,-24}},
      color={175,0,0},
      thickness=0.5));
...
end TestStratifiedHotWaterStorage_L1;
