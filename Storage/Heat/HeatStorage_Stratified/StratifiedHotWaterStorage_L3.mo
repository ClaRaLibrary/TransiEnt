...
model StratifiedHotWaterStorage_L3 "Model of one dimensional hot water storage"
...
if used_Ports_Int == 0 then Utilities.get_Ports_noSolar(
 ...
 elseif used_Ports_Int == 10 then Utilities.get_Ports_noSolar_addPorts(
  ...
  Modelica.Blocks.Interfaces.RealOutput minTemperature "Minimum Temperature in the storage"
        ...
        rotation=0,
        origin={-56,76})));
  Modelica.Blocks.Interfaces.RealOutput storedEnergie
  ...
  Modelica.Blocks.Interfaces.RealOutput heatOutFlow annotation (Placement(transformation(extent={{96,60},{112,76}})));
 ...
 // Volumes
  Basics.Fluid_Volume[nSeg] Tank_Volume(
    ...
    each p_nom=p_nom,
    each h_nom=h_nom,
    h_start=h_start) "Fluid Volumes the storage is divided in"
    annotation (Placement(transformation(
        extent={{-15,-16},{15,16}},
        rotation=-90,
        origin={-22,-7})));
    ...
    annotation (Placement(transformation(extent={{56,-20},{32,4}})));
  ...
  Basics.PortGeometry HeaPortsGeometry[nHeatPorts](
    ...
    annotation (Placement(transformation(extent={{-36,80},{-24,92}})));
                                        //if Use_HeatPorts
 ...
 Basics.FlowSplit flowSplit(
   ...
   final layerTemperatures=Tank_Volume[solarInPortGeometry.segment -
    ...
    annotation (Placement(transformation(extent={{-70,20},{-48,-2}})));
  ...
  Basics.PortGeometry Geo_outletCHP(
    ...
    height_port=0.2)
    annotation (Placement(transformation(extent={{-88,-28},{-76,-16}})));
  ...
  Basics.SolarInPortGeometry solarInPortGeometry(
    ...
    height_port=1,
    length_port=0.3)
    annotation (Placement(transformation(extent={{-88,-8},{-76,4}})));
...
algorithm
   storedEnergie := sum(Tank_Volume.h*Tank_Volume.m);
...
end StratifiedHotWaterStorage_L3;
