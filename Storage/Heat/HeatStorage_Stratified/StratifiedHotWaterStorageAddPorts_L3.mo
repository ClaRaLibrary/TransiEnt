...
model StratifiedHotWaterStorageAddPorts_L3 "Model of one dimensional hot water storage with additional Ports"
...
//___________________________________________________________________________//
  ...
  parameter SI.Temperature maxTemperature_allowed = 371.15 "maximal allowed temperature in tank";
  parameter SI.Temperature minTemperature_allowed = 283.15 "minimal allowed temperature in tank";

  parameter SI.Temperature refTemperature_max = 363.15 "Reference temperature for maximum filling level";
  parameter SI.Temperature refTemperature= 293.15 "Reference temperature for minimal filling level ";
  ...
  // Fluid ports for fluid from solar something else
  constant Boolean Add_FluidPorts = true "Add an arbitrary number of fluid ports"
  ...
  parameter Integer nAdditionalFluidPorts = 2 "Choose the number of additional fluid ports"
    annotation(Dialog(group="Ports", enable=Add_FluidPorts));        //if Add_FluidPorts
  ...
  final parameter Integer used_Ports_Int = Use_Solar_Int + Add_FluidPorts_Int "Desribes which ports are in use";
...
if used_Ports_Int == 0 then Utilities.get_Ports_noSolar(
 ...
 elseif used_Ports_Int == 10 then Utilities.get_Ports_noSolar_addPorts(
  ...
  TransiEnt.Basics.Interfaces.Thermal.FluidPortIn inletCHP(final Medium=medium) annotation (Placement(transformation(extent={{-114,52},{-94,72}}), iconTransformation(extent={{-110,50},{-90,70}})));
  ...
  TransiEnt.Basics.Interfaces.Thermal.FluidPortIn inletSolar(final Medium=medium) if
                            Use_Solar annotation (Placement(transformation(extent={{-112,0},{-92,20}}), iconTransformation(extent={{-110,20},{-90,40}})));
  TransiEnt.Basics.Interfaces.Thermal.FluidPortOut outletSolar(final Medium=medium) if
                            Use_Solar annotation (Placement(transformation(extent={{-110,-92},{-90,-72}}), iconTransformation(extent={{-110,-50},{-90,-30}})));
 ...
 // Real
     Modelica.Blocks.Interfaces.RealOutput maxTemperature "maximum temperature in tank"
                                  annotation (Placement(transformation(
        extent={{8,-8},{-8,8}},
        rotation=0,
        origin={-56,90})));
     Modelica.Blocks.Interfaces.RealOutput minTemperature "Minimum Temperature in the storage"
                                         annotation (Placement(transformation(
        extent={{8,-8},{-8,8}},
        rotation=0,
        origin={-56,76})));
     Modelica.Blocks.Interfaces.RealOutput storedEnergy "stored energy"
    annotation (Placement(transformation(extent={{-48,52},{-64,68}})));
     Modelica.Blocks.Interfaces.RealOutput averageTemperature "Temperature average based on the mass"
                                            annotation (Placement(
        transformation(
        extent={{8,-8},{-8,8}},
        rotation=0,
        origin={-56,44})));
    Modelica.Blocks.Interfaces.RealOutput heatOutFlow annotation (Placement(transformation(extent={{96,56},{112,72}})));

    Modelica.Blocks.Interfaces.RealOutput relativeStorageFilling "values between zero and one" annotation (Placement(transformation(extent={{96,80},{112,96}})));
  ...
  // _____________________________________________
  // replaceable models
  replaceable Basics.Cylindric_Geometry Geometry(
    height=2,
    volume=1,
    final nSeg=nSeg)
              constrainedby Basics.Partial_Geometry "Defines the base geometry of the storage"
                                               annotation (choicesAllMatching=true,
      Placement(transformation(extent={{-38,-78},{-22,-62}})));

  replaceable Basics.FlatWall_HeatConduction ConductanceTop(
    final A_heat= Geometry.A_top) constrainedby TransiEnt.Storage.Heat.HeatStorage_Stratified.Basics.Partial_HeatTransfer    "Defines the heat conductance at the top of the storage"
                                                             annotation (
    Dialog(group="Heat Losses"),
    choicesAllMatching=true,
    Placement(transformation(extent={{74,60},{90,76}})));

  replaceable Basics.CylindricWall_HeatTransfer[nSeg] ConductanceWall(each final
            height=Geometry.height_Seg,
      each radius=Geometry.diameter/2)
    constrainedby TransiEnt.Storage.Heat.HeatStorage_Stratified.Basics.Partial_HeatTransfer    "Defines the heat conductance at the side walls of the storage"
                                                                    annotation (
    Dialog(group="Heat Losses"),
    choicesAllMatching=true,
    Placement(transformation(extent={{74,36},{90,52}})));

  replaceable Basics.FlatWall_HeatConduction ConductanceBottom(
    final A_heat= Geometry.A_bottom) constrainedby TransiEnt.Storage.Heat.HeatStorage_Stratified.Basics.Partial_HeatTransfer    "Defines the heat conductance at the bottom of the storage"
                                                                annotation (
    Dialog(group="Heat Losses"),
    choicesAllMatching=true,
 ...
 // Volumes
  Basics.Fluid_Volume[nSeg] Tank_Volume(
    h_start={180000,190000,180000,190000,160000,170000,180000,160000,170000,190000},
    final nPorts=PortCountVector,
    final V_const=Geometry.volume_Seg,
    each final medium=medium) "Fluid Volumes the storage is divided in"
    annotation (Placement(transformation(
        extent={{-15,-16},{15,16}},
        rotation=-90,
        origin={-20,-7})));
    ...
    final medium=medium) "Models buoyancy due adding heat flows"
    annotation (Placement(transformation(extent={{60,-20},{36,4}})));

    Basics.ThermalConductor_Fluid[nSeg - 1] ConFluid(
    final A_heat=Geometry.A_cross,
    each final height_Seg=Geometry.height_Seg,
    each k=0.67) "Thermal conductance between fluid segments"
    ...
    final m= nSeg+2) "Collects the thermal lossen from top, sidewall and bottom"
         annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=180,
        origin={60,78})));

  Basics.PortGeometry HeaPortsGeometry[nHeatPorts](
    each final nSeg=nSeg,
    each final height_segment=Geometry.height_Seg,
    height_port={1}) "Heights the heatPorts are connected to the storage"
    annotation (Placement(transformation(extent={{-38,80},{-24,92}})));

    //if Use_Solar
 Basics.FlowSplit flowSplit(
   final nOutPorts=solarInPortGeometry.nSolar,
   final layerTemperatures=Tank_Volume[solarInPortGeometry.segment -
        solarInPortGeometry.nSolar + 1:solarInPortGeometry.segment].T,
    final medium=medium) if
    Use_Solar "distributes the fluid flow from the solar heating to the storage segments "
    annotation (Placement(transformation(extent={{-62,22},{-44,4}})));

    // Port geometries
  Basics.PortGeometry Geo_outletCHP(
    final nSeg=nSeg,
    final height_segment=Geometry.height_Seg,
    height_port=0.2)
    annotation (Placement(transformation(extent={{-92,-32},{-80,-20}})));
  Basics.PortGeometry Geo_inletCHP(
    final nSeg=nSeg,
    final height_segment=Geometry.height_Seg,
    height_port=1.6)
    annotation (Placement(transformation(extent={{-92,68},{-80,80}})));
  Basics.PortGeometry Geo_inletGrid(
    final nSeg=nSeg,
    final height_segment=Geometry.height_Seg,
    height_port=0.2)
    annotation (Placement(transformation(extent={{80,-86},{94,-72}})));
  Basics.PortGeometry Geo_outletGrid(
    final nSeg=nSeg,
    final height_segment=Geometry.height_Seg,
    height_port=1.6)
    annotation (Placement(transformation(extent={{80,-10},{94,4}})));
  Basics.PortGeometry Geo_outletSolar(
    final nSeg=nSeg,
    final height_segment=Geometry.height_Seg,
    height_port=0.2)
    annotation (Placement(transformation(extent={{-92,-72},{-80,-60}})));

    // Port geometry solar in
  Basics.SolarInPortGeometry solarInPortGeometry(
    final nSeg=nSeg,
    final height_segment=Geometry.height_Seg,
    height_port=1,
    length_port=0.3)
    annotation (Placement(transformation(extent={{-92,-12},{-80,0}})));

    // Port geometries if Add_Ports
    Basics.PortGeometry[nAdditionalFluidPorts] Geo_addPorts(
    each final  nSeg=nSeg,
    each final  height_segment=Geometry.height_Seg,
    height_port={1,1})
    annotation (Placement(transformation(extent={{-32,-100},{-16,-84}})));
  ...
  TransiEnt.Components.Statistics.Collectors.LocalCollectors.StorageCost collectCosts_Storage(
    ...
    Delta_E_n=(maxTemperature_allowed - refTemperature)*Geometry.volume*1e3*4.2e3) annotation (Placement(transformation(extent={{40,-100},{60,-80}})));
  ...
  // _____________________________________________
algorithm
   storedEnergy := sum(Tank_Volume.h*Tank_Volume.m)-h_ref*sum(Tank_Volume.m);
...
equation
  ...
  assert(nSeg>nSeg_min-1,"Use more tank segments. The allowed minimum is 4 segments");
...
if Use_Solar then
  ...
  connect(Buo.heatPort, Tank_Volume.heatPort) annotation (Line(
      points={{37.2,-7.28},{30,-7.28},{30,-7},{-4.64,-7}},
      color={191,0,0},
      smooth=Smooth.None));
...
end StratifiedHotWaterStorageAddPorts_L3;
