...
model StratifiedHotWaterStorage_L3 "Model of one dimensional hot water storage, constant media properties"
...
//___________________________________________________________________________//
  ...
  parameter SI.Length h_primaryHeatSourceWaterIn = h "Index of control volume containing heater (1=bottom, n=top)" annotation(Dialog(group="Heating Units"));
  parameter SI.Length h_primaryHeatSourceWaterOut = 0 "Index of control volume containing heater (1=bottom, n=top)" annotation(Dialog(group="Heating Units"));
  parameter SI.Length h_gridSideWaterIn = 0 "Index of control volume containing grid side inflow (1=bottom, n=top)" annotation(Dialog(group="Heating Units"));
  parameter SI.Length h_gridSideWaterOut = h "Index of control volume containing grid side outflow (1=bottom, n=top)" annotation(Dialog(group="Heating Units"));
  ...
  final parameter SI.ThermalConductance[N_cv] G = cat(1,{(A_wall+A_top)*U_bottom}, fill(A_wall*U_wall, N_cv-2), {(A_wall+A_bottom)*U_top});
  ...
  final parameter Integer i_primaryHeatSourceWaterIn=Base.getIdxFromHeight(
      N_cv,
      h,
      h_primaryHeatSourceWaterIn) "Index of control volume containing heater (1=bottom, n=top)" annotation (Dialog(group="Heating Units"));
  final parameter Integer i_primaryHeatSourceWaterOut=Base.getIdxFromHeight(
      N_cv,
      h,
      h_primaryHeatSourceWaterOut) "Index of control volume containing heater (1=bottom, n=top)" annotation (Dialog(group="Heating Units"));
  final parameter Integer i_gridSideWaterIn=Base.getIdxFromHeight(
      N_cv,
      h,
      h_gridSideWaterIn) "Index of control volume containing grid side inflow (1=bottom, n=top)" annotation (Dialog(group="Heating Units"));
  final parameter Integer i_gridSideWaterOut=Base.getIdxFromHeight(
      N_cv,
      h,
      h_gridSideWaterOut) "Index of control volume containing grid side outflow (1=bottom, n=top)" annotation (Dialog(group="Heating Units"));
  ...
  TransiEnt.Components.Statistics.Collectors.LocalCollectors.StorageCost collectStorageCosts(
    ...
    Q_flow_is=Q_flow_unload) annotation (Placement(transformation(extent={{40,-100},{60,-80}})));
  ...
  TransiEnt.Basics.Interfaces.Thermal.FluidPortOut waterPortOut_primaryHeatSource(final Medium=medium) annotation (Placement(transformation(extent={{-110,-70},{-90,-50}}), iconTransformation(extent={{-110,-20},{-90,0}})));

  // Consumer Side connectors

  TransiEnt.Basics.Interfaces.Thermal.FluidPortIn waterPortIn_Grid(final Medium=medium) annotation (Placement(transformation(extent={{90,-70},{110,-50}}), iconTransformation(extent={{88,-40},{108,-20}})));
  TransiEnt.Basics.Interfaces.Thermal.FluidPortOut waterPortOut_Grid(final Medium=medium) annotation (Placement(transformation(extent={{90,50},{110,70}}), iconTransformation(extent={{88,26},{108,46}})));
