...
ClaRa.Components.Utilities.Blocks.VariableGradientLimiter
  ...
  Modelica.Blocks.Sources.Constant CapacityLimit1(k=0)     annotation (Placement(transformation(extent={{-50,-39},{-44,-32}})));
  Basics.Blocks.FirstOrder              plantDynamic(Tau=T_plant, initOption=4) if                             T_plant<>0   annotation (Placement(transformation(extent={{76,-4},{84,4}})));
equation
  ...
  connect(CapacityLimit.y, P_is_external.u3) annotation (Line(
      points={{37.65,-41},{48,-41},{48,-20},{50,-20},{50,-19.4},{50.4,-19.4}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(P_is_external.y, P_is_internal.u1) annotation (Line(points={{68.8,-13},{74,-13},{74,-34},{64,-34},{64,-64.8},{74.6,-64.8}},
                                                                                                                    color={0,0,127}));
  ...
  if use_PowerRateLimiter==true then
    ...
    connect(PowerRateLimit.y, P_is_external.u1) annotation (Line(points={{-7,52},{36,52},{36,-6.6},{50.4,-6.6}}, color={0,0,127}));
  else
