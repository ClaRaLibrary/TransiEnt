...
<p>The TransiEnt Library prodides adapters which allow the user to connect models from the Modelica Standard Library to system models within TransiEnt, e.g. in <a href=\"TransiEnt.Basics.Adapters.Check.TestEPP_to_QS\">TestEPP_to_QS</a> the connection of <a href=\"Modelica.Electrical.QuasiStationary\">MSL.Quasistationary</a> components is illustrated.</p>
<h4>Conventions</h4>
<h5>Sign conventions</h5>
<p>The sign conventions in the TransiEnt Library follow the consumer side convention: In consumers the flow variables are positive and in producers the flow variables are negative. If setpoints are passed to the models, they are supposed to have the same presign as the flow variable in the connector. For example a power plant model gets negative setpoints and has a negative power flow in its connector. Storage models get positive values for loading (increase of the state of charge). These conventions are illustrated in the picture below.</p>
...
<p>1. Purpose of model. In which simulation context will the model be used? What questions and physical effects shall be analysed with the model?</p>
...
<ul>
<li>L1: Models are based on characteristic lines and / or transfer functions, e.g. <a href=\"TransiEnt.Storage.Base.GenericStorage\">GenericStorage</a>. This results in an idealised model, which shows physical behaviour. The model definition may be derived either from analytic solutions to the underlying physics or from phenomenological considerations. Applicability is limited to the validity of the simplification process. Non-physical behaviour may occur otherwise.</li>
<li>L2: Models are based on balance equations. These equations are spatially averaged over the component (lumped volume), e.g. <a href=\"TransiEnt.Storage.Heat.HotWaterStorage_L2\">HotWaterStorage_L1</a></li>
<li>L3: Models are by construction subdivided into a fixed number of spatial zones. The spatial localisation of these zones is not necessarily fixed and can vary dynamically. For each zone a set of balance equations is used and the model properties (e.g. media data) are averaged zone-wise, e.g. <a href=\"TransiEnt.Storage.Heat.HeatStorageStratified_constProp.StratifiedHotWaterStorage_L3\">StratifiedHotWaterStorage</a></li>
</ul>
</html>"));
end UsersGuide;
